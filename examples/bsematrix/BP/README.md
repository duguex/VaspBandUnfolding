...
The data in this directory supports several validation levels:

- **pw_only**: L2 -- eigenvalues agree with VASP within a few meV for
  all three interaction cases (direct-only, exchange-only, both).
  Suitable for citing as "agrees with VASP".
- **paw_orth_only / paw_full**: L1--L2 -- the tables above document the
  current gap. Do not cite as validated without a separate accuracy
  statement.
- **Finite-q** (all modes): L0--L1 -- not validated. Do not cite as L2.

See [](../../../FEATURES.md) X1--X3 for current levels.
