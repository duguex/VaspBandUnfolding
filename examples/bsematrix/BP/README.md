...
The data in this directory supports several validation levels (see also `docs/CITATION_POLICY.md`):

- **pw_only**: **l2-partial** — eigenvalues agree with VASP within a few meV for
  the three interaction cases (direct-only, exchange-only, both). Prefer this mode;
  it is the **CLI default**. Suitable to cite as partial agreement with VASP, not
  as bit-identical kernels.
- **paw_orth_only / paw_full**: **quarantine** — tables document residuals. Do not
  cite as validated.
- **Finite-q** (all modes): **quarantine** — not validated.

See [`FEATURES.md`](../../../FEATURES.md) X1–X3 for current levels.
